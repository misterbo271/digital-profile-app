...
export default class Web extends Base {
    ...
    setOptions(navigation, route) {
        ...
        navigation.setOptions({
            ...
            headerRight: () => (
                <CBTouchableOpacity style={[appStyles.action, {marginRight: 5}]} define={'none'} onPress={this.onMenu}>
    ...
    load() {
        AsyncStorage.getItem('savedCookies').then(async value => {
            const object = JsonUtil.parseJsonString(value);
            let cookieString = JsonUtil.jsonCookieToCookieString(object);
            const PHPSESSID = await AsyncStorage.getItem('PHPSESSID');
            if (PHPSESSID) {
                cookieString += `PHPSESSID=${PHPSESSID};`;
            }
            const headers = this.defaultParam?.headers || {};
            this.setState({
                source: {
                    uri: this.defaultParam?.uri || 'https://google.com',
                    headers: cookieString ? {...headers, Cookie: cookieString} : headers
                },
    ...
    renderError = (domain, code, description) => {
        return (
            <CBView style={[appStyles.overlay]} define={'overlay'}>
                <CBImage containerStyle={[appStyles.image, {width: moderateScale(180), height: moderateScale(180)}]} source={ImageUtil.getImage('bg_warning')} resizeMode={'contain'}/>
